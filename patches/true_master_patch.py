...
target_str = '''        if config is None:
            if "decoder.mid.block_1.mix_factor" in sd:'''

if target_str in content:
    # 1. Inject the new block
    content = content.replace(target_str, ltxv_clean_block)
    
    # 2. Remove the old block
    old_pattern = r'\s+elif "decoder\.up_blocks\.0\.res_blocks\.0\.conv1\.conv\.weight" in sd or "decoder\.up_blocks\.0\.resnets\.0\.conv1\.conv\.weight" in sd: #lightricks ltxv.*?self\.working_dtypes = \[torch\.bfloat16, torch\.float32\]\n'
    content = re.sub(old_pattern, '\n', content, flags=re.DOTALL)
    
    with open(path, 'w') as f:
        f.write(content)
    print("True Master Patch Applied Successfully!")
else:
    print("Target string not found! Patch failed.")
