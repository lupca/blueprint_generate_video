...
Hệ thống hoạt động như một cụm **Silent Server** giao tiếp qua giao thức HTTP REST và WebSockets ở cổng mặc định `8188`. Các file cấu hình đồ thị JSON (`workflows/workflow_flux_api.json` và `workflows/workflow_ltxv_api.json`) đóng vai trò là cấu trúc điều hướng tham số.

---

## 0. Khởi chạy Server ComfyUI (Bắt buộc)
Trước khi gọi bất kỳ API nào, server ComfyUI phải được khởi chạy:
```bash
...
async function generateImage(prompt, outputPath) {
    ...
    const workflow = JSON.parse(fs.readFileSync('./workflows/workflow_flux_api.json', 'utf8'));
...
Nạp file `workflows/workflow_ltxv_api.json` và điều chỉnh các node đầu vào:
